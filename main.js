...
function validateCred(creditNumber) {
  let total = 0;
  for (let i = creditNumber.length - 1; i >= 0; i--) {
    let currentValue = creditNumber[i];
    if ((creditNumber.length - 1 - i) % 2 === 1){
        currentValue *= 2;
            if (currentValue > 9) {
              currentValue -= 9;
            }}
...
function findInvalidCards (newArray) {
  for (let i = 0; i < newArray.length; i++) {
    let newCard = newArray[i];
